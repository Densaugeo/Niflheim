...
const PACKET_TYPE_CELL_UPDATE: u8 = 4;
const PACKET_TYPE_CELL_CACHE: u8 = 2;
const PACKET_TYPE_AGENT_CACHE: u8 = 3;
const PACKET_TYPE_REGION_PROPERTIES: u8 = 1;
...
fn main () {
  ...
  loop {
    ...
    if msg.len() == 5 {
      ...
      match msg[4] {
        PACKET_TYPE_CELL_CACHE        => cache_sender.send(&packetize_cell_cache(&map), 0).unwrap(),
        PACKET_TYPE_AGENT_CACHE       => cache_sender.send(&packetize_agent_cache(&some_agents), 0).unwrap(),
        PACKET_TYPE_REGION_PROPERTIES => cache_sender.send(&packetize_region_properties(), 0).unwrap(),
...
fn packetize_cell_update() -> Vec<u8> {
  let mut packet = make_base_packet(PACKET_TYPE_CELL_UPDATE);
...
fn packetize_cell_cache(map: &[[Cell; HEIGHT]; WIDTH]) -> Vec<u8> {
  let mut packet = make_base_packet(PACKET_TYPE_CELL_CACHE);
...
fn packetize_agent_cache(cache: &Vec<Agent>) -> Vec<u8> {
  let mut packet = make_base_packet(PACKET_TYPE_AGENT_CACHE);
...
fn packetize_region_properties() -> Vec<u8> {
  let mut packet = make_base_packet(PACKET_TYPE_REGION_PROPERTIES);
